...
def send_about(message):
    ...
    with open('template_text/about.txt', mode='r', encoding='utf-8') as f:
        ...
        about = temp.substitute(
            STUDENT_NAME="Benjamin Sitompul",
            BATCH_ACADEMY="Qoppa Night Online",
            GITHUB_REPO_LINK="https://github.com/fitzben?tab=repositories"
        )
...
def ask_id_summary(message):
    ...
    sent = bot.send_message(
        chat_id, 'Choose campaign to be summarized:', reply_markup=markup)
...
def send_summary(message):
    ...
    if selected_campaign_id in unique_campaign:
        ...
        start_date = df_campaign['reporting_date'].min().strftime(
            format='%d %b %Y')
        end_date = df_campaign['reporting_date'].max().strftime(
            format='%d %b %Y')
...
def send_plot(message):
    ...
    if selected_campaign_id in unique_campaign:
        ...
        df_plot = df_campaign.pivot_table(
            ...
            aggfunc='sum',
        ).reindex(columns=['spent', 'approved_conversion'])
